...
- **严格发布隔离**：发布流程统一为 `output/github-export/` 脱敏导出仓库，不再使用源仓库分支或独立发布历史直接发布。
...
- **连续抓取多个演员**：完成一个演员的文件夹创建后，程序会回到演员 ID 输入提示，可继续输入下一个演员 ID。
- **复用浏览器会话**：连续抓取时复用同一个 Chrome 和 Cookie 会话，不再每个演员重新启动浏览器。
- **新增退出输入**：演员 ID 输入处支持 `q`、`quit`、`exit` 退出程序；直接回车仍使用配置文件默认 ID。
- **优化 CMD 输出**：减少满屏 `#` 分隔线，改为带缩进层级的分区标题、步骤、细节输出，并使用多种符号展示包含关系；过长内容会提前换行，续行保持同级缩进。
...
- **启动时直接输入演员 ID**：CMD 中直接输入演员号码；直接回车使用配置文件默认 ID，不再先输入 `1`、`2` 选择来源。
