...
第一阶段整改已经收敛 `.gitignore`、迁移真实 Cookie、补充脱敏示例并将
非输出内容纳入本地 Git。更新后的全局 `AGENTS.md` 进一步规定：
已完成计划必须归档；含私有历史的源仓库不得直接发布；发布必须通过
`output/github-export/` 中的脱敏导出仓库；push 后源仓库、导出仓库和
远程都只能保留 `main`。当前仓库仍保留旧导出分支和旧发布说明，因此
需要完成第二阶段收尾。
